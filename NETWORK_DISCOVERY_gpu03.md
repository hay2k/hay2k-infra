...
(`cluster_ed25519`). Read-only — nothing changed on `gpu-03`.
...
| SSH from gpu-01 | **OK** (passwordless via `cluster_ed25519`; alias `gpu-03`) |

**Identical hardware/OS to `gpu-01`/`gpu-02`.** DNS list differs (only the KT
resolver). See CLUSTER_NETWORK_SUMMARY.md.
